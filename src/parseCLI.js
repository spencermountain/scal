...
function setKey(obj, keys, value, flags) {
  let o = obj;
...
const parseCLI = function (input, opts) {
  ...
  function setArg(key, val, arg) {
    ...
    setKey(argv, key.split('.'), value, flags);

    (aliases[key] || []).forEach(function (x) {
      setKey(argv, x.split('.'), value, flags);
    });
  }
  ...
  Object.keys(defaults).forEach(function (key) {
    if (!hasKey(argv, key.split('.'))) {
      setKey(argv, key.split('.'), defaults[key], flags);

      (aliases[key] || []).forEach(function (x) {
        setKey(argv, x.split('.'), defaults[key], flags);
      });
    }
  });
